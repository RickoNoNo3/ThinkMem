...
6. **实时通信**: HTTP模式支持SSE和WebSocket，提供实时更新
...
└────────┬────────┘    └────────┬────────┘    └────────┬────────┘
         │                     │                     │
         │ MCP stdio           │ HTTP/SSE/WebSocket │ HTTP API
         ▼                     ▼                     ▼
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│ ThinkMemServer  │    │  HttpSSEServer  │    │  HttpSSEServer  │
│   (MCP协议)      │    │  (HTTP协议)      │    │  (HTTP协议)      │
└────────┬────────┘    └────────┬────────┘    └────────┬────────┘
         │                     │                     │
         └─────────────────────┼─────────────────────┘
...
- **HttpSSEServer**: HTTP协议实现，支持REST API、SSE、WebSocket
...
- **ListMemory**: 完整实现，支持数组/队列/栈三种模式
- **存储系统**: JSON文件持久化，支持备份恢复、并发控制

#### 服务器实现
- **MCP Stdio服务器**: 26个工具全部实现，完整错误处理
- **HTTP SSE服务器**: REST API + WebSocket + SSE，多客户端支持
- **协议适配**: 统一业务逻辑，不同通信方式
...
### HTTP模式流程
```
Web客户端 → HTTP请求 → Express路由 → 业务逻辑 → JsonStorage → File System
           ← HTTP响应 ← JSON格式 ← 处理结果 ← 数据读取

WebSocket客户端 → Socket.IO事件 → 业务逻辑 → JsonStorage
              ← Socket.IO响应 ← 处理结果

SSE客户端 → EventSource连接 → SSE流 → 实时事件推送
```
